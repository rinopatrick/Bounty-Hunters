...
import { ProviderCache } from "./services/ProviderCache.js";
...
export const providerCacheHealthRouteLayer = HttpRouter.add(
  "GET",
  "/health/cache",
  Effect.gen(function* () {
    const status = yield* ProviderCache.pipe(
      Effect.flatMap((svc) => svc.healthCheck),
    );
